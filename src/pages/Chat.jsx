...
function formatCourseTime(course) {
  const day = course.day || '요일 미정';
  const periods = Array.isArray(course.periods) ? course.periods.join('') : '';
  return periods ? `${day}${periods}` : day;
}

function compactCourse(course) {
  const rating = Number(course.etaReview?.rating);
  const review = Number.isFinite(rating) ? `, 에타 ${rating.toFixed(2)}` : '';
  return `${course.courseName || course.name || '과목명 미정'}(${course.professor || '교수 미정'}, ${formatCourseTime(course)}${review})`;
}
...
function buildDirectAnswer(question, user, metrics) {
  ...
  if (normalized.includes('졸업요건') || normalized.includes('졸업')) {
    ...
    return {
      text: `${user?.name || 'SSAI 학생'}님 기준으로 졸업까지 ${remainingCredits}학점 남았어요. 지금 우선순위는 ${nextActions || '남은 영역 없음'}입니다. 이번 학기에는 전공필수와 외국어인증을 먼저 마무리하고, 남은 학점은 SSAI 전공선택으로 채우는 플랜이 가장 깔끔해요.`,
  ...
  if (normalized.includes('전공필수')) {
    const requiredText = requiredCourses.length
      ? requiredCourses.map(compactCourse).join(', ')
      : '소셜데이터프로그래밍기초, 자료구조, 데이터베이스';
    return {
      text: `전공필수는 현재 ${requiredArea?.earned || 0}/${requiredArea?.required || 0}개 완료 기준이에요. 2026-2에 바로 노릴 과목은 ${requiredText}입니다. 우선순위는 데이터베이스, 소셜데이터프로그래밍기초, 자료구조 순으로 두면 졸업요건과 시간표 안정성이 좋아요.`,
  ...
  if (normalized.includes('수강신청') || normalized.includes('대안') || normalized.includes('실패')) {
    const backupText = (backupCourses.length ? backupCourses : official2026SecondSemesterCourses)
      ...
      .join(', ');
    return {
      text: `수강신청 대안은 이렇게 잡으면 돼요. 1순위는 전공필수 3개, 2순위는 평점/부담이 좋은 ${backupText}, 3순위는 금공강·팀플 조건에 맞춰 자동 생성한 Plan A입니다. 실패한 과목이 생기면 시간표 탭의 ‘수강신청 결과 반영’에서 과목을 누르면 대체 플랜으로 바로 바꿀 수 있어요.`,
      sources: answerSources,
    };
  }
