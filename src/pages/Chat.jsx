...
const answerSources = ['SSAI 공식 졸업요건', '한국외대 2026-2 SSAI 강의시간표'];

function courseNames(courses) {
  return courses.map((course) => course.courseName || course.name).filter(Boolean).join(', ');
}

function areaLine(area, unit = '학점') {
  if (!area) return '';
  const need = Math.max(Number(area.required || 0) - Number(area.earned || 0), 0);
  return `${area.label} ${area.earned}/${area.required}${unit}${need ? `, ${need}${unit} 부족` : ', 충족'}`;
}

function buildDirectAnswer(question, user, metrics) {
  const normalized = question.replace(/\s/g, '');
  const areas = Array.isArray(metrics?.areas) ? metrics.areas : [];
  const remainingCredits = Number.isFinite(Number(metrics?.remainingCredits)) ? Number(metrics.remainingCredits) : 0;
  const primaryArea = areas.find((area) => area.key === 'primaryMajor');
  const liberalArtsArea = areas.find((area) => area.key === 'liberalArts');
  const requiredArea = areas.find((area) => area.key === 'requiredCourses');
  const weakAreas = areas.filter((area) => area.status !== '충족');
  ...
  if (normalized.includes('졸업요건') || normalized.includes('졸업')) {
    const nextActions = weakAreas.slice(0, 2).map((area) => areaLine(area, area.key === 'requiredCourses' ? '개' : '학점')).join(' · ');
    return {
      text: `현재 ${metrics?.earnedCredits || 0}/${metrics?.totalRequired || 134}학점, 졸업까지 ${remainingCredits}학점 남았어요. ${nextActions || '주요 요건은 충족 상태예요.'}`,
      sources: ['SSAI 공식 졸업요건', 'SSAI 공식 교육과정'],
    };
  }

  if (normalized.includes('전공필수')) {
    const requiredText = requiredCourses.length
      ? courseNames(requiredCourses)
      : '소셜데이터프로그래밍기초, 자료구조, 데이터베이스';
    return {
      text: `전공필수는 ${requiredArea?.earned || 0}/${requiredArea?.required || 0}개 이수 상태예요. 2026-2 추천: ${requiredText}.`,
      sources: answerSources,
    };
  }

  if (normalized.includes('추천과목') || normalized.includes('다음학기') || normalized.includes('데이터')) {
    const picked = dataCourses.length ? dataCourses.slice(0, 4) : official2026SecondSemesterCourses.slice(0, 4);
    return {
      text: `추천 과목: ${courseNames(picked)}.`,
      sources: answerSources,
    };
  }

  if (normalized.includes('수강신청') || normalized.includes('대안') || normalized.includes('실패')) {
    const backupText = (backupCourses.length ? backupCourses : official2026SecondSemesterCourses)
      .slice(0, 3)
      .map((course) => course.courseName)
      .join(', ');
    return {
      text: `대안 과목: ${backupText}. 시간표 탭에서 실패 과목을 누르면 대체 플랜으로 바뀌어요.`,
      sources: answerSources,
    };
  }

  if (normalized.includes('전공학점') || normalized.includes('전공')) {
    return {
      text: areaLine(primaryArea) || '전공 이수학점 정보가 아직 없어요. 프로필에서 전공 이수학점을 입력해주세요.',
      sources: ['SSAI 공식 졸업요건'],
    };
  }

  if (normalized.includes('교양')) {
    return {
      text: areaLine(liberalArtsArea) || '교양 이수학점 정보가 아직 없어요. 프로필에서 교양 이수학점을 입력해주세요.',
      sources: ['SSAI 공식 졸업요건'],
    };
  }
...
export default function Chat({ chat, setChat, user, metrics, events = [] }) {
  ...
  const visibleChat = safeChat.length ? safeChat : [{ role: 'assistant', text: answer, sources: [] }];
  ...
  const ask = (question) => {
    ...
    const earnedCredits = Number.isFinite(Number(metrics?.earnedCredits)) ? Number(metrics.earnedCredits) : 0;
    const remainingCredits = Number.isFinite(Number(metrics?.remainingCredits)) ? Number(metrics.remainingCredits) : 0;
    let text = String(result?.text || result?.a || '플래니가 답변을 준비하지 못했어요. 질문을 조금 다르게 입력하면 바로 다시 정리해드릴게요.')
      .replace('108학점', `${earnedCredits}학점`)
      .replace('26학점', `${remainingCredits}학점`);
    ...
    if (typeof setChat === 'function') {
      setChat((current) => [
        ...
        { role: 'assistant', text, sources: result?.sources || [] },
      ]);
    }
    setInput('');
  };
